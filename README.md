...
先下降（模型越来越好）到某个点最低（模型最佳）再上升（开始过拟合）。模型最优点 = 验证集 loss 最低 + 训练集 loss 不再大幅下降。
欠拟合：训练误差都很高     过拟合：训练误差很低，但验证误差变高
...
1. FPR 高的时候，看 TPR 能不能更高（比下三滥基线高）  

想象你把阈值调得很低 —— 几乎所有东西都被判为 “正例”。
...
但此时模型应该至少做到：
把大多数正例也抓出来（TPR 也很高，尽量接近 1）

如果 FPR 已经高了，但是 TPR 还不高，那模型就很烂。

好模型：FPR 高时 TPR 逼近 1
坏模型：FPR 高时 TPR 还是低 → 那你误报多，真阳又没抓出来，纯纯废物。  

🎯 2. FPR 低的时候，看 TPR 能维持多高（这是能力体现）

FPR 很低说明你非常严格、很保守（不轻易判“正”）
此时应该：  

如果模型很强 → TPR 仍然不错（比如 0.6, 0.7）

如果模型很弱 → TPR 直接掉到 0（几乎抓不到正例）  

所以：

能在“误报低”的前提下仍然抓到正例 → 强模型

只要你不误报就抓不到正例 → 弱模型

这也就是 ROC 中最关键的一段：

从 (FPR=0, TPR=0) 到 (FPR≈0.2, TPR↑) 这段越弯向左上角，其实越强。
...
最后分母上的M*N就不多说了，就是一正一负样本总共能够组成的对数。
...
- [ ] [2-1-1  你是怎样理解“特征”？](#2-1-1)
特征就是 ——
数据中能够帮助模型区分不同情况的“可量化信息”。

换句话说，特征是模型的“眼睛”，帮模型看世界。

对图像来说：边缘、纹理、颜色

对 EMG 来说：RMS、均方根、频谱能量、放电频率

对时间序列来说：趋势、周期、变化率

一句话总结：
👉 特征是把原始复杂数据提炼成“有意义的数字表达”。
- [ ] [2-1-2  给定场景和问题，你如何设计特征？（特征工程方法论）](#2-1-2)
      我一般遵循一套固定的流程：
...
C. 高级特征（Task-specific）
按任务定制：

识别疲劳 → α/β band power ratio

肌肉招募模式 → MU firing rate

手势识别 → 通道间 correlation / 图结构
...
看各组之间是否有显著差异（t-test, Mann-Whitney）
...
例子：
SEMG 信号有坏掉的通道 → 直接插值 + 通道剔除。

② 处理异常值

通过 3σ 法、箱线图 IQR 检测

频谱异常 → notch 或 band-pass 处理
...
例子：
做多受试者 EMG 分析时，信号必须统一 2048Hz，否则特征会乱套。
...
好嘞 Yun，我给你讲一个**既适合算法岗面试、又让你自己也能一眼吃透的 LDA（Linear Discriminant Analysis）原理 + 训练过程**。
保证你 2 分钟讲完，对方直接点头。

我讲的是**机器学习里的 LDA（Fisher LDA）**，不是主题模型的 LDA（Latent Dirichlet Allocation）。

---

# ⭐ **什么是 LDA？（一句话）**

> **LDA 是一种监督学习的线性降维方法，目标是：让不同类别之间尽可能远，让同类别内部尽可能近。**

它不是“为了压缩维度而降维”，而是：

👉 **为了提高分类效果而降维**（尤其适合 EEG/EMG 手势分类、ERP 检测、BCI competition 那类任务）

---

# ⭐ **LDA 模型的核心思想（两个矩阵 + 一个优化）**

LDA 的关键是两个矩阵：

---

## **① 类内散度矩阵（Sw：Within-class scatter）**
...
公式：
$$
S_w = \sum_{k=1}^K \sum_{x_i \in class\ k} (x_i - \mu_k)(x_i - \mu_k)^T
$$
...
## **② 类间散度矩阵（Sb：Between-class scatter）**
...
我们需要一个投影方向 **w**（或者多个方向），让：

$$
\text{类间距离最大} \quad \text{类内距离最小}
$$
...
## **Step 4：求解广义特征值问题 Sb w = λ Sw w**

得到 w₁, w₂, …（投影方向）

---

## **Step 5：选最大几个特征值对应的方向**
...
# ⭐ **LDA 为什么特别适合 BCI？（你加上这个别人会觉得你真的懂）**

* BCI（尤其是 EEG/EMG 手势识别）类别数少（典型 2～10 类）
* 噪声大 → LDA 能最大化类间距离、压缩噪声
* 数据维度高（128 channels × time）→ LDA 必须降维
* 多数传统算法（CSP + LDA）是 BCI 经典 pipeline

你可以说一句：

> “LDA 本质上是最大化类间可分性，这非常适合 EEG/EMG 这种噪声大但类别结构明确的信号分类任务。比如 CSP-LDA 在 BCI Competition 里是经典组合。”
...
![img](https://github.com/terry11zzzz/ML/blob/main/svm.jpeg)
...
**核函数定义**：设  $\mathcal{X}$  是输入空间，又设  $\mathcal{H}$  为特征空间，如果存在一个从  $\mathcal{X}$ 
 到  $\mathcal{H}$  的映射
...
$$
f(x) = \text{sign}(
\sum_{i=1}^{N_s} a_i^{*} y_i (x_i \cdot x + 1)^p + b^{\star}
)
$$
...
$$
f(x)=\text{sign}\left(\sum_{i=1}^{N_{s}} a_{i}^{\star} y_{i} \exp \left(-\frac{\|x-x_i\|^{2}}{2 \sigma^{2}}\right)+b^{\star}\right)
$$
